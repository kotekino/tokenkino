...
class TKAux(BaseModel):
    ...
    # modality (2026-07-14, the Socratic dialogue; □ added M4 2026-07-16): "possibility" when a
    # possibility modal (can/could/may/might) scopes the clause, "necessity" for must. A modal
    # claim is NOT a crisp assertion — ◇P must never enter the consistency kernel as P (◇P ∧ ◇¬P
    # is trivially consistent; the drop cost the author -0.2 as "self-contradiction") and □P never
    # mints an is_a edge (necessity is not bare assertion — its consumer arc is deferred).
